...
export default function Nav({ currentView, onNavigate }) {
  ...
  const abbrev = addr => addr ? `${addr.slice(0, 4)}…${addr.slice(-4)}` : "";
  ...
  return (
    <nav style={{
      ...
      >
        ...
        <span style={{ fontFamily: "var(--mono)", fontWeight: 700, fontSize: 13, letterSpacing: "0.06em", color: "var(--text)" }}>
        ...
        {ROUTES.map(r => (
          <button
            ...
            style={{
              ...
              color: currentView === r.view ? "var(--accent)" : "var(--text-3)",
              background: currentView === r.view ? "var(--accent-dim)" : "transparent",
      ...
      {state.wallet ? (
        ...
        <Button size="sm" onClick={() => onNavigate("connect")}>Connect Wallet</Button>
      )}
    </nav>
  );
}